...
function MobileTrackList({
  ...
  // rowRefs is owned by the parent and written through setRowRef; it is
  // accepted here so the mobile list and the desktop list share one
  // source of truth for the rows the physics loop transforms.
  void rowRefs
  return (
    <div
      ref={listViewportRef}
  ...
  return (
    <button
      ...
      style={{
        ...
        justifyContent: "center",
        cursor: "pointer",
        background: accent ? `linear-gradient(140deg, ${accent}, ${accent}bb)` : "rgba(255,255,255,0.09)",
        border: accent ? "none" : `1px solid ${fgMutedVar(0.22)}`,
        boxShadow: accent ? `0 6px 22px ${accent}66` : "none",
        color: accent ? "#04070f" : fgVar,
...
// The real, functional transport: prev / play / next, the now-playing
// line, and — only while the video theme is on — the ambient volume
// slider. The slider's max is MAX_VIDEO_VOLUME, not 1, so the video can
// never be raised past the scroll click.
function PlayerControls({
  ...
  const accent = track?.colorA ?? CYAN
  const playSize = compact ? 52 : 48
  const stepSize = compact ? 40 : 38
  ...
  return (
    <div
      style={{
        ...
        right: 0,
        bottom: 0,
        padding: compact ? "0 18px 26px" : "0 18px 20px",
        display: "flex",
        flexDirection: "column",
        alignItems: "center",
        gap: 10,
        // A soft upward wash so the transport keeps contrast over any
        // frame of the video without putting a hard panel on top of it.
        background: `linear-gradient(180deg, ${cardVar(0)} 0%, ${cardVar(0.55)} 60%, ${cardVar(0.86)} 100%)`,
        pointerEvents: "none",
      }}
    >
      <div style={{ pointerEvents: "auto", width: "100%", display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: compact ? 22 : 18 }}>
          <ControlButton onClick={onPrev} label="Previous track" size={stepSize}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
              <path d="M6 6h2v12H6zM20 6v12L10 12z" />
            </svg>
          </ControlButton>

          <ControlButton onClick={onPlay} label={isPlaying ? "Pause" : "Play"} size={playSize} accent={accent}>
            {isPlaying ? (
          ...
          <ControlButton onClick={onNext} label="Next track" size={stepSize}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
        ...
        {showVideoControls && (
          <div style={{ display: "flex", alignItems: "center", gap: 10, width: compact ? "78%" : 190, maxWidth: "100%" }}>
            <button
              className="mh-focusable"
              ...
              style={{
                ...
                alignItems: "center",
                cursor: "pointer",
                color: videoSoundOn ? CYAN : fgMutedVar(0.45),
                flexShrink: 0,
